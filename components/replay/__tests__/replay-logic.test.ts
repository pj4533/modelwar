...
describe('formatCycle', () => {
  ...
  it('formats maxCycles', () => {
    expect(formatCycle(252000)).toBe('252,000');
  });
...
describe('computeProgress', () => {
  ...
  it('returns 0 when cycle is 0', () => {
    const state = makeState({ cycle: 0, maxCycles: 252000, endCycle: null });
    expect(computeProgress(state)).toBe(0);
  });

  it('uses maxCycles when endCycle is null', () => {
    const state = makeState({ cycle: 126000, maxCycles: 252000, endCycle: null });
    expect(computeProgress(state)).toBe(50);
  });

  it('uses endCycle when set (fixes the 19% bug)', () => {
    const state = makeState({ cycle: 95160, maxCycles: 252000, endCycle: 95160 });
    expect(computeProgress(state)).toBe(100);
  });

  it('shows partial progress toward endCycle', () => {
    const state = makeState({ cycle: 47580, maxCycles: 252000, endCycle: 95160 });
...
describe('createInitialState', () => {
  it('returns loading state with defaults', () => {
    ...
    expect(state.maxCycles).toBe(252000);
...
describe('reducer', () => {
  ...
  describe('FETCH_SUCCESS', () => {
    it('sets maxCycles', () => {
      const state = reducer(initialState, { type: 'FETCH_SUCCESS', maxCycles: 252000 });
      expect(state.maxCycles).toBe(252000);
    });
  });
  ...
  describe('ROUND_END', () => {
    ...
    it('handles tie result', () => {
      ...
      expect(state.endCycle).toBe(252000);
    });
